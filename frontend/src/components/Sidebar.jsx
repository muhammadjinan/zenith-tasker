...
const Sidebar = ({ pages, onCreatePage, activePageId, onSelectPage, onDeletePage, onReorder, user, onLogout, activeView, onViewChange }) => {
      ...
      <div className="w-64 h-screen flex flex-col relative z-20 bg-slate-950/95 border-r border-white/5">
        ...
        <nav className="flex-1 overflow-y-auto px-3 pb-4">
          ...
          <div className="mb-1">
            <div className="flex items-center px-3 py-2">
              <button
                ...
                className="flex-1 flex items-center gap-2 cursor-pointer hover:text-slate-200 transition-colors rounded-lg hover:bg-white/5 -ml-2 pl-2 py-1"
              >
                <svg
                  className={`w-3 h-3 text-slate-500 transition-transform duration-200 ${isTasksExpanded ? 'rotate-90' : ''}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
            ...
            {isTasksExpanded && (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 mb-1 rounded-lg transition-all ${activeView === 'tasks'
                  ? 'bg-cyan-500/10 text-cyan-400 border-l-[3px] border-cyan-400'
                  : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'
                  }`}
              >
          ...
          <div className="flex items-center px-3 py-2">
            ...
            >
              <svg
                className={`w-3 h-3 text-slate-500 transition-transform duration-200 ${isPagesExpanded ? 'rotate-90' : ''}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
